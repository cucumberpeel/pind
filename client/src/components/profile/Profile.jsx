...
function Profile() {
    ...
    useEffect(() => {
        if (!isSelf && user && profile) {
            axios.get(`/api/friends/status/${profile?.user_id}`, { withCredentials: true })
    ...
    const handleFriendRequest = () => {
        axios.post(`/api/friends/request`, { receiver_id: profile?.user_id }, { withCredentials: true })
    ...
    const handleAccept = () => {
        axios.post(`/api/friends/accept`, { sender_id: profile?.user_id }, { withCredentials: true })
        .then(() => setFriendStatus('friends'))
        .catch(err => console.error(err))
    };

    const handleDecline = () => {
        axios.post(`/api/friends/decline`, { sender_id: profile?.user_id }, { withCredentials: true })
    ...
    useEffect(() => {
        axios.get(`/api/user/${username}/streams`)
